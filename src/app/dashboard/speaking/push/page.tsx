...
export default function SpeakingPage() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-9rem)] flex-col">
      {/* live WebRTC lesson — the flagship; this page stays as push-to-talk fallback */}
      <a href="/dashboard/speaking/live" className="mb-4 flex items-center justify-between rounded-2xl bg-gradient-to-r from-[var(--color-brand)] to-[var(--color-brand-2)] px-5 py-4 text-white shadow-md transition-transform hover:scale-[1.01]">
        <span className="flex items-center gap-3">
          <span className="text-xl">🎙️</span>
          <span>
            <span className="block text-sm font-bold">{pick(locale, { ru: { t: "Живой урок с преподавателем" }, en: { t: "Live lesson with the teacher" }, tr: { t: "Öğretmenle canlı ders" }, kk: { t: "Ұстазбен жанды сабақ" } }).t}</span>
            <span className="block text-xs opacity-85">{pick(locale, { ru: { t: "Реальный разговор, исправления на лету · TÖMER Konuşma" }, en: { t: "Real conversation, live corrections · TÖMER Konuşma" }, tr: { t: "Gerçek diyalog, anında düzeltme · TÖMER Konuşma" }, kk: { t: "Нағыз әңгіме, лезде түзету · TÖMER Konuşma" } }).t}</span>
          </span>
        </span>
