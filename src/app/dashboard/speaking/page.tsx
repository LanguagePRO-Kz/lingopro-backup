...
/* Урок ОДИН — выбора режима здесь больше нет (решение основателя 16.08.2026).
 * Вариант урока решает сервер по уровню (A0-A2 → «Фундамент» без экзамена и
 * баллов, B1+ → обычный урок), экзаменационные форматы живут в Konuşma. */
...
const T = {
  ru: {
    ...
    listening: "Слушает тебя — говори", speaking: "Преподаватель говорит…", minutes: "Уроков в этом месяце", purchased: "докупленные",
    ...
    errNoMinutes: "Уроки этого месяца закончились. Лимит обновится с новым месяцем подписки — или докупи уроки прямо сейчас.", buyMinutes: "Докупить уроки",
  ...
  en: {
    ...
    listening: "Listening — go ahead", speaking: "Teacher is speaking…", minutes: "Lessons this month", purchased: "purchased",
    ...
    errNoMinutes: "You have used all lessons for this month. The limit renews with your next subscription month — or top up now.", buyMinutes: "Buy lessons",
  ...
  tr: {
    ...
    listening: "Dinliyor — konuş", speaking: "Öğretmen konuşuyor…", minutes: "Bu ayki dersler", purchased: "satın alınan",
    ...
    errNoMinutes: "Bu ayki derslerin bitti. Limit yeni abonelik ayında yenilenir — ya da hemen ek ders al.", buyMinutes: "Ders al",
  ...
  kk: {
    ...
    listening: "Тыңдап тұр — сөйлей бер", speaking: "Ұстаз сөйлеп жатыр…", minutes: "Осы айдағы сабақтар", purchased: "сатып алынған",
    ...
    errNoMinutes: "Осы айдағы сабақтар бітті. Лимит жаңа жазылым айында жаңарады — немесе қазір қосымша сабақ сатып ал.", buyMinutes: "Сабақ сатып алу",
...
function LiveLesson() {
  ...
  // plan-engine deep link (?focus=izafet,conditionals) — задача «голосовой
  // урок» из плана дня закрепляет темы недели. Режим больше НЕ передаётся:
  // урок один, и его вариант выбирает сервер по уровню.
        ...
        if (res?.ok) {
          ...
          if ((d.minutes ?? 0) > 0) {
            void awardXp("speaking_test", XP.SPEAKING_TEST, {
              ...
              metadata: { conversationId: convIdRef.current, seconds: d.seconds, mode: "lesson" },
            });
  ...
  async function start() {
    ...
    try {
      const res = await fetch("/api/voice/session", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ mode: "lesson", feedbackLang: locale, voiceId: voice, focusTopics: focusOverrideRef.current ?? undefined }),
      });
      if (!res.ok) {
    ...
    setResolvedMode((data as { resolvedMode?: string }).resolvedMode ?? "lesson");
    ...
    <div>
      ...
      {phase === "idle" && (
        <div className="glass mt-5 rounded-3xl p-6">
          <div className="text-xs font-medium text-[var(--color-muted)]">{c.voice}</div>
