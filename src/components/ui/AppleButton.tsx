...
/** Apple auth button — visual placeholder until real OAuth is wired up. */
export function AppleButton({ label, onClick }: { label: string; onClick?: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center justify-center gap-3 rounded-full bg-black px-6 py-3.5 text-sm font-medium text-white shadow-sm transition-all hover:bg-black/85 hover:shadow-md"
    >
