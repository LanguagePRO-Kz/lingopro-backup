...
console.log("— контекст: потолок и честные секции —");
{
  ...
  const fat = snap({
    ...
    lastVoice: { endedAt: `${d(-1)}T18:00:00Z`, minutes: 9, topicsWorked: ["izafet", "conditionals"], errorCount: 3, criteriaTotal: 12, invalidReason: null },
    topicsClosed: 4,
  });
...
console.log("— таймзона: «вчера» в сутках студента —");
{
  ...
  const sNoReview = snap({
    lastVoice: { endedAt: `${TODAY}T05:00:00Z`, minutes: 3, topicsWorked: [], errorCount: 0, criteriaTotal: null, invalidReason: null },
  });
  ...
  const sClean = snap({
    lastVoice: { endedAt: `${TODAY}T05:00:00Z`, minutes: 5, topicsWorked: ["present_iyor"], errorCount: 0, criteriaTotal: 15, invalidReason: null },
  });
