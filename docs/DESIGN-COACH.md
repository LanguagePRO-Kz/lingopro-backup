...
## 14. Маскот Ahu — СДЕЛАНО 2026-07-13 (+ титулы, символ роста, эволюция тона)

Реализовано по утверждённому плану (hashed-cooking-hellman):
- **`src/components/mascot/AhuMascot.tsx`** — параметрический SVG, 8 пресетов
  = CoachStateId (`MASCOT_PRESETS` экспортирован), моргание/дыхание/морф,
  кивок по пропу `nudge` (AhuCoach инкрементит на `lp:daily-updated`);
- **`src/components/mascot/GrowthTree.tsx`** — 4 стадии по реальному уровню
  (`growthStage`); проп под детализацию титулами заложен;
- **`src/lib/coach/titles.ts`** — 15 честных титулов: пороги = утверждённая
  таблица (Adım Atan снижен правкой основателя до 8/12 A1 — ~2-я неделя);
  `computeTitle` (лестница, юнит-тесты: XP-гринд не двигает, барем C1
  обязателен для TÖMER Ustası), `buildCareerStats` (7 запросов полной
  истории, только в брифе), `titleFactsTr` (факты для AI-поздравления),
  `titleCongratsText` (шаблон-фолбэк ×4);
- **бриф**: считает карьеру, детектит апгрейд (заработанный ранг > хранимого),
  присуждает (profiles.title_slug, НЕ отбирается), поздравляет конкретными
  цифрами (day_key дня становится `TITLE:{id}`), отдаёт `level` и `title`
  в ответе; до миграции 0011 деградирует честно (титулы просто не даются);
- **тон по уровню** (persona.ts): REHBER→ANTRENÖR→PARTNER→MESLEKTAŞ→DOST;
  турецкий ведёт только В ЧАТЕ на B2/C1 (SON KONTROL уровнезависим), бриф
  строг на всех уровнях — страж языка не тронут;
- **UI**: маскот в AhuCoach (реальное состояние) и шапке чата (ON_TRACK),
  дерево+чип титула у подписи, титул в лидерборде (RPC расширен в 0011);
- **`/dev/mascot`** — галерея пресетов/стадий/бейджей (в проде 404).

Проверено: юнит-тесты (титулы/тон) зелёные; живой бриф на посеянном пропуске —
маскот грустит, титул присуждён, текст поздравил реальными цифрами («3 активных
дня и 13 заданий»); скриншоты галереи и дашборда глазами.
Миграция `0011_titles.sql` (title_slug + get_leaderboard с титулом) — ждёт
применения основателем; до неё титулы не присуждаются, всё остальное живёт.
TODO следующему этапу: титул в профиле/настройках; детализация дерева по
титулу; замена ассетов художником (1:1 по пресетам).

### (история) первоначальный план маскота
